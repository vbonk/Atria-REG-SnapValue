...
import { UploadCloud, File, X, Loader, Wand, Calendar, CircleDollarSign, Wrench, MapPin, ArrowRight, ArrowLeft, Download, RefreshCw, Phone, Mail, Globe, Replace, Tags, CheckCircle, Eye, Clock, AlertCircle } from 'lucide-react';
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
            <div className="bg-white rounded-lg shadow-xl max-w-md w-full p-6 relative">
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                    ...
                    {error && (
                        <div className="bg-red-50 border border-red-200 text-red-600 px-4 py-2 rounded-md text-sm flex items-start">
...
export default function App() {
    ...
    const copyToClipboard = async (text, successMessage = 'Copied to clipboard!') => {
        try {
            ...
            updateState({ error: '' }); // Clear any existing errors
            // Could show a toast here if you have a toast component
            console.info(successMessage);
        } catch (err) {
            // Fallback for Safari/HTTP contexts
            ...
            <div className="bg-white rounded-lg shadow-md overflow-hidden border border-gray-200 not-prose">
                ...
                <div className="p-6">
                    ...
                    <div className="bg-gray-50 rounded-lg p-4 grid grid-cols-1 sm:grid-cols-3 gap-4 text-sm">
                        <div className="flex items-center">
                            <CircleDollarSign className="w-5 h-5 mr-2 text-gray-500" aria-hidden="true" />
                            <div>
            ...
            <header className="bg-white shadow-md">
                <div className="container mx-auto px-4 sm:px-6 lg:px-8">
                    <div className="flex items-center justify-center py-4">
                        <img src="/logo.png" alt="Atria Logo" className="h-10 mr-4"/>
                ...
                <div className="grid grid-cols-1 gap-8">
                                                ...
                                                {files.map(fileObj => (
                                                    <div key={fileObj.id} className="flex items-center justify-between bg-gray-100 p-2 rounded-md">
                                                        <div className="flex items-center truncate">
                                                            <File className="h-5 w-5 text-gray-500 mr-2 flex-shrink-0" aria-hidden="true" />
                                        ...
                                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                                            {files.map((fileObj, index) => (
                                                <div key={fileObj.id} className="space-y-2">
                                                    ...
                                                    <div className="relative">
                                                        <Tags className="absolute left-3 top-1/2 -translate-y-1/2 h-5 w-5 text-gray-400" aria-hidden="true" />
                                                        <select 
                                                            value={fileObj.tag} 
                                                            onChange={(e) => handleTagChange(index, e.target.value)} 
                                                            className="w-full pl-10 pr-4 py-2 border border-gray-300 rounded-md focus:ring-red-700 focus:border-red-700 appearance-none"
                                ...
                                {step === 3 && (
                                    <div>
                                        <div className="grid grid-cols-1 md:grid-cols-2 gap-y-4 gap-x-4 max-w-xl mx-auto">
                                            <div className="md:col-span-1"><label htmlFor="city" className="block text-sm font-medium text-gray-700 mb-1">City</label><div className="relative"><MapPin className="absolute left-3 top-1/2 -translate-y-1/2 h-5 w-5 text-gray-400" aria-hidden="true" /><input id="city" type="text" placeholder="e.g., Woodbury" value={city} onChange={e => updateState({ city: e.target.value })} className="w-full pl-10 pr-4 py-2 border border-gray-300 rounded-md focus:ring-red-700 focus:border-red-700" /></div></div>
                                            <div className="md:col-span-1"><label htmlFor="state" className="block text-sm font-medium text-gray-700 mb-1">State</label><div className="relative"><MapPin className="absolute left-3 top-1/2 -translate-y-1/2 h-5 w-5 text-gray-400" aria-hidden="true" /><input id="state" type="text" placeholder="e.g., Minnesota" value={stateName} onChange={e => updateState({ stateName: e.target.value })} className="w-full pl-10 pr-4 py-2 border border-gray-300 rounded-md focus:ring-red-700 focus:border-red-700" /></div></div>
                                            <div className="md:col-span-1"><label htmlFor="timeline" className="block text-sm font-medium text-gray-700 mb-1">Selling Timeline</label><div className="relative"><Calendar className="absolute left-3 top-1/2 -translate-y-1/2 h-5 w-5 text-gray-400" aria-hidden="true" /><select id="timeline" value={timeline} onChange={e => updateState({ timeline: e.target.value })} className="w-full pl-10 pr-4 py-2 border border-gray-300 rounded-md focus:ring-red-700 focus:border-red-700 appearance-none"><option>Within one month</option><option>1-3 months</option><option>3-6 months</option><option>Just curious</option></select></div></div>
                                            <div className="md:col-span-1"><label htmlFor="budget" className="block text-sm font-medium text-gray-700 mb-1">Improvement Budget ($)</label><div className="relative"><CircleDollarSign className="absolute left-3 top-1/2 -translate-y-1/2 h-5 w-5 text-gray-400" aria-hidden="true" /><input id="budget" type="number" placeholder="e.g., 1000" value={budget} onChange={e => updateState({ budget: e.target.value })} className="w-full pl-10 pr-4 py-2 border border-gray-300 rounded-md focus:ring-red-700 focus:border-red-700" /></div></div>
                                            <div className="md:col-span-2"><label htmlFor="work-preference" className="block text-sm font-medium text-gray-700 mb-1">Work Preference</label><div className="relative"><Wrench className="absolute left-3 top-1/2 -translate-y-1/2 h-5 w-5 text-gray-400" aria-hidden="true" /><select id="work-preference" value={workPreference} onChange={e => updateState({ workPreference: e.target.value })} className="w-full pl-10 pr-4 py-2 border border-gray-300 rounded-md focus:ring-red-700 focus:border-red-700 appearance-none"><option>A mix of DIY and Professional work</option><option>DIY only</option><option>Professional work only</option></select></div></div>
                                        </div>
                                    </div>
                                )}
                                {step === 4 && (
                             ...
                             <div className="mt-8">
                                 {error && (
                                    <div className="flex items-start text-red-500 text-sm mb-4">
                                        ...
                                        {lastRequestId && (
                                            <button 
                                                onClick={() => copyToClipboard(lastRequestId, 'Request ID copied!')}
                                 ...
                                 <div className="flex gap-4">
                                    {step > 1 && (<button onClick={prevStep} className="w-full bg-gray-200 hover:bg-gray-300 text-gray-800 font-bold py-3 px-4 rounded-lg flex items-center justify-center transition-all"><ArrowLeft className="h-5 w-5 mr-2" aria-hidden="true" />Back</button>)}
                                    {step < 4 && (<button onClick={nextStep} className="w-full text-white font-bold py-3 px-4 rounded-lg flex items-center justify-center transition-all" style={{backgroundColor: '#B91C1C'}}>Next<ArrowRight className="h-5 w-5 ml-2" aria-hidden="true" /></button>)}
                                    {step === 4 && (
                                         <button onClick={generateReport} disabled={isLoading} className="w-full text-white font-bold py-3 px-4 rounded-lg flex items-center justify-center transition-all disabled:bg-gray-400" style={{backgroundColor: '#B91C1C'}}>
                                            {isLoading ? (<><Loader className="animate-spin h-5 w-5 mr-3" aria-hidden="true" />Generating Report...</>) : (<><Wand className="h-5 w-5 mr-3" aria-hidden="true" />Generate Report</>)}
                                        </button>
                                    )}
                                 </div>
                            </div>
                    ...
                    {(report || isLoading) && (
                        <div className="bg-white p-6 rounded-lg shadow-lg">
                            ...
                            {isLoading && (<div className="flex flex-col items-center justify-center min-h-[400px]"><Loader className="animate-spin h-16 w-16" style={{color: '#B91C1C'}} aria-hidden="true" /><p className="mt-4 text-gray-600 text-center">{loadingMessage}</p></div>)}
                            {report && (
                                 <div className="space-y-6">
                                    ...
                                    <div className="bg-white rounded-lg shadow-lg mt-8 p-6 border border-gray-200">
                                        ...
                                        <p className="text-gray-600 mb-4 text-sm">Download your professional PDF report with personalized recommendations.</p>
                                        <div className="space-y-4">
                                            <button onClick={downloadPDF} className="w-full bg-green-600 hover:bg-green-700 text-white font-bold py-3 px-4 rounded-lg flex items-center justify-center transition-all">
                                                <Download className="h-5 w-5 mr-2" aria-hidden="true" />Download PDF Report
                                            </button>
                                            <button onClick={resetApp} className="w-full bg-gray-200 hover:bg-gray-300 text-gray-800 font-bold py-3 px-4 rounded-lg flex items-center justify-center transition-all">
                                                <RefreshCw className="h-5 w-5 mr-2" aria-hidden="true" />Start New Report
                                            </button>
                                        </div>
                                    </div>
                                </div>
